...
async function tmdbFetch<T>(path: string, params: Record<string, string | number | boolean> = {}): Promise<T> {
  if (!API_KEY) throw new Error("TMDB API key missing. Add VITE_TMDB_API_KEY to your .env file.");
...
export const tmdb = {
  ...
  popularMovies: (page = 1) => tmdbFetch<{ results: TMDBMovie[] }>(`/movie/popular`, { page }),
  popularTV: (page = 1) => tmdbFetch<{ results: TMDBMovie[] }>(`/tv/popular`, { page }),
  search: (query: string) =>
